...
When users enter the payDebt command to settle a balance, payDebtCommandParser will parse the arguments, and return payDebtCommand.
Next, payDebtCommand will call its method execute(). After a payDebt command is executed, a calculateUpdatedBalance() method will be called.
...
the event as a transaction of type "paydebt".
An AddressBookChangeEvent will be posted to allow the storage to handle this event and save the new transaction and balances of the involved Persons into the storage file.
The UI will also handle this event by adding the new transaction to the TransactionList Panel and updating the balance of each Person that was involved in the transaction.

==== Design Considerations
===== Aspect: Implementation of `payDebtCommand`
