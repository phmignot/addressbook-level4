...
From the diagram above, when users enter the command addTransaction payer/ Payer a/ Amount d/ Description payee/ Payee to add a new a Transaction in their address book, AddTransactionCommandParser will parse the arguments, and return AddTransactionCommand.
...
The diagram below shows how the addTransaction operation work in the logic component
image::AddTransactionSequenceDiagram.png[width="800"]

==== Add Transaction Event process
.Component interactions for `addTransaction` command (part 2)
image::SDforDeletePersonEventHandling.png[width="800"]
With reference to the diagram above, after addTransaction method is executed, the AddTransactionCommandParser, as part of the Logic component, will calculate a new balance for the payer and each payee.
...
The UI will also handle this event by adding the new transaction to the TransactionList Panel and updating the balance of each Person.

==== Design Considerations
Why it is implemented that way

Instead of saving each transaction details to every person involved, a new transaction object is created and added in transactionlist object when addTransaction command is executed. This allows all the new transactions to be stored in the hand disk and can be easily retrieved when the user requests for transaction history details.
...
* **Alternative 2:** Use `HistoryManager` for undo/redo
** Pros: We do not need to maintain a separate stack, and just reuse what is already in the codebase.
** Cons: Requires dealing with commands that have already be
// tag::dataencryption[]
...
From the diagram above, when users enter the command select INDEX to select the first person in the persons list, SelectCommandParser will parse the argument, and return SelectCommand.
Next, SelectCommand will call its method execute(). After select command is executed, an JumpToListRequestEvent is raised.
...
There are two ways to raise this event. The first one is by using this select command, the second one is by directly clicking on the PersonCard in the Addressbook.
.Component interactions for `PersonPanelSelectionChanged` event
image::PersonPanelSelectionChangedEventDiagram.png[width="800"]
With reference to the diagram above, the PersonPanelSelectionChanged event is handled by the Model, that will call the UpdateFilteredTransactionList method in the addressbook, with the predicate to keep only transactions related with the person selected.
The FilteredTransactionList will be updated, the UI will display the new list.

==== PersonPanelNoSelection Event process
In order to raise this event, the selection has to be cleared. There are several ways, it is possible to type another command that will clear the selection, such as the `find` command, or by directly deselecting a selected PersonCard using "Ctrl + Clic".

.Component interactions for `PersonPanelSelectionChanged` event
image::PersonPanelSelectionChangedEventDiagram.png[width="800"]
With reference to the diagram above, the PersonPanelNoSelection event is handled by the Model, that will call the UpdateFilteredTransactionList method in the addressbook, with the predicate to keep every transactions.
The FilteredTransactionList will be updated, the UI will display the new list.

==== Why it is implemented that way

This implementation can satisfy both user used and unused to Command Line. This implementation used events that are raised if needed, so there is no need to check regularly if the selection has changed. That implementation saves the application from wasting of resource by un.

=== [Proposed] Pay Debt feature
==== Pay Debt Command Implementation
When users enter the command payDebt c/ Creditor d/ Debtor /a Amount to settle a balance, payDebtCommandParser will parse the arguments, and return payDebtCommand.
Next, payDebtCommand will call its method execute(). After a payDebt command is executed, a payDebt() method will be called.

==== Pay Debt Event process
After the payDebt method is executed, the PayDebtCommandParser, as part of the Logic component, will subtract the amount from the balance of the debtor and record
the event as a transaction.
An AddressBookChangeEvent will be posted to allow the storage to handle this event and save the new transaction and balances of the involved Persons into the hard disk.
The UI will also handle this event by adding the new transaction to the TransactionList Panel and updating the balance of each Person.
...
* Major feature: Allow user to categorise their spending into different categories, so as to allow the ease of tracking what they spent most on
* Minor feature: Allow user to edit their added transaction
...
* Major feature: Allow users to settle their balance by recording a payment via a "record payment" command
* Minor feature: Allow users to split their expenses using evenly, by percentage, and by units
...
|`* * *` |user |see how much I owe or am owed in total|
...
1.  User requests to create group
2.  SmartSplit prompts user for the name of the new group
3.  User enters the desired name
4.  SmartSplit creates the new group with the name and designates the user as the admin user for that group
+
Use case ends.

[discrete]
=== Use case: Add members to group
...
1.  User requests to add transaction
...
+
[none]
** 3a1. SmartSplit shows an error message and prompts the user to try again.
+
