...
|`* * *` |admin user |add people to a group |

|`* * *` |admin user |delete people from a group |remove those are are no longer involved with the shared expenses

|`* * *` |admin user |add an expense |make a record of it

|`* * *` |user |sort my expense into different categories |keep better track of my expenses

|`* * *` |user |edit an expense |correct any mistakes I may have made

|`* * *` |user |delete an expense |remove a mistakenly entered or unwanted expense

|`* * *` |user |specify which group members were involved with an expense |

|`* * *` |user |split an expense evenly over the group members who were involved by default |

|`* * *` |user |split an expense by units |properly attribute unevenly shared expenses to the individuals that were involved

|`* * *` |user |see all transactions recorded by group members in a table |see a complete history of what was paid

|`* * *` |user |see a summary of the expenses paid per person |figure out how much each person owes or is owed

|`* * *` |user |see my total balance |know how much I owe or can expect to be paid back

|`* * *` |user |record payment to another group member |settle my balance with that individual
...
.  Should be able to hold up to 100 users without a noticeable sluggishness in performance for typical usage.
...
.  The size of group cannot be smaller than 2.
