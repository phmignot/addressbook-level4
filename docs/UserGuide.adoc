...
Deletes the specified person from SmartSplit, the person needs to be cleared from any debt. +
Format: `delete INDEX`
...
* The person has no debts with anyone.
****
...
Adds a transaction between one payer to one/mutiple payees
...
By clicking on the person on the person panel list, transactions that are relevant to that person
...
and all history of transactions will be re-displayed
...
=== Listing all transactions : `listTransactions`[coming v1.4]

Shows a list of all transactions in SmartSplit. +
Format: `listTransactions`

=== Locating transactions by name: `findTransaction`[coming v1.4]
