...
public class EditPersonCommandSystemTest extends AddressBookSystemTest {
    ...
    public void edit() throws Exception {
        ...
        Person editedPerson = new PersonBuilder().withName(VALID_NAME_BOB).withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withBalance(VALID_BALANCE_BOB).withTags(VALID_TAG_HUSBAND).build();
        ...
        command = EditPersonCommand.COMMAND_WORD + " " + index.getOneBased() + " " + NAME_DESC_JACK;
        personToEdit = getModel().getFilteredPersonList().get(index.getZeroBased());
        editedPerson = new PersonBuilder(personToEdit).withName(VALID_NAME_JACK).build();
        assertCommandSuccess(command, index, editedPerson);
        ...
        executeCommand(PersonUtil.getAddPersonCommand(BOB));
